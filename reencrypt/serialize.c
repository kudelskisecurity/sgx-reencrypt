...
reencrypt_status key_serialize(const struct keydata_t *k, uint8_t **blob,
    ...
    memcpy(temp + offsetof(struct keydata_t, keys_from), k->keys_from,
           k->n_keys_from * sizeof(key_id));
    memcpy(temp + offsetof(struct keydata_t, keys_from) +
               k->n_keys_from * sizeof(key_id),
           k->keys_to, k->n_keys_to * sizeof(key_id));
    memcpy(temp + offsetof(struct keydata_t, keys_from) +
               (k->n_keys_from + k->n_keys_to) * sizeof(key_id),
           k->authorized_clients, k->n_authorized_clients * sizeof(client_id));
...
reencrypt_status key_deserialize(struct keydata_t **k, const uint8_t *blob,
    ...
    temp->authorized_clients =
        malloc(temp->n_authorized_clients * sizeof(client_id));
    ...
    memcpy(temp->authorized_clients,
           blob + offsetof(struct keydata_t, keys_from) +
